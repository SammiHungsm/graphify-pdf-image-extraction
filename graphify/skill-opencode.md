...
> **graphify needs no mandatory API key. Never ask the user for one, and never block on one.** Code is extracted structurally (AST) with no LLM and no key at all — a code-only corpus (the common `/graphify .` on a repo) skips semantic extraction entirely, so it needs nothing here: go straight to Part A and skip Part B. Semantic extraction (only for docs, papers, and images) uses MiniMax if `MINIMAX_API_KEY` is already set, Gemini if `GEMINI_API_KEY`/`GOOGLE_API_KEY` is already set, otherwise the host agent itself is the LLM. graphify does **not** read `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, or any other provider key. If you catch yourself about to prompt for, wait on, or stop because of a missing API key, that is a misread of this skill — proceed without one.

**Before semantic extraction:** check whether `MINIMAX_API_KEY` is set. If not set, print this one-liner to the user:
...
Print it once, then continue — do not wait for the user to supply a key. If `MINIMAX_API_KEY` IS set, use `graphify.llm.extract_corpus_parallel(files, backend="minimax")` for semantic extraction instead of dispatching subagents. The default model is `MiniMax-M2`; set `GRAPHIFY_MINIMAX_MODEL` or pass `--model` in headless CLI flows to override it. If `MINIMAX_API_KEY` is unset but `GEMINI_API_KEY` / `GOOGLE_API_KEY` is set, fall back to `backend="gemini"` (legacy).

> **No other API keys are read.** When neither `MINIMAX_API_KEY` nor `GEMINI_API_KEY`/`GOOGLE_API_KEY` are set, semantic extraction falls to the host agent itself — the running session is the LLM. On a host that dispatches subagents, dispatch them as written in Part B. On a host that runs the CLI directly in a terminal and cannot dispatch subagents, do not stall: a code-only corpus has no semantic work, so write the empty semantic file (Part B "Fast path") and continue to Part C; for a corpus with docs/papers/images, either set a MiniMax/Gemini key or extract those inline yourself, but in no case prompt for `ANTHROPIC_API_KEY` — that prompt is a misread of this skill.
