...
> **graphify needs no API key. Never ask the user for one, and never block on one.** Code is extracted structurally (AST) with no LLM and no key at all — a code-only corpus (the common `/graphify .` on a repo) skips semantic extraction entirely, so it needs nothing here: go straight to Part A and skip Part B. Semantic extraction (only for docs, papers, and images) uses Minimax **only if** `MINIMAX_API_KEY` is already set; otherwise the host agent itself is the LLM. graphify does **not** read `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, or any other provider key. If you catch yourself about to prompt for, wait on, or stop because of a missing API key, that is a misread of this skill — proceed without one.

**Before semantic extraction:** check whether `MINIMAX_API_KEY` or `GOOGLE_API_KEY` is set. If neither is set, print this one-liner to the user:
> Tip: set `MINIMAX_API_KEY` or `GOOGLE_API_KEY` to use MiniMax for semantic extraction (`pip install 'graphifyy[minimax]'`).

Print it once, then continue — do not wait for the user to supply a key. If `MINIMAX_API_KEY` or `GOOGLE_API_KEY` IS set, use `graphify.llm.extract_corpus_parallel(files, backend="minimax")` for semantic extraction instead of dispatching subagents. The default MiniMax model is `MiniMax-M3`; set `MINIMAX_API_KEY` or pass `--model` in headless CLI flows to override it.

> **No other API keys are read.** When `MINIMAX_API_KEY`/`GOOGLE_API_KEY` are unset, semantic extraction falls to the host agent itself — the running session is the LLM. On a host that dispatches subagents (e.g. Claude Code), dispatch them as written in Part B. On a host that runs the CLI directly in a terminal and cannot dispatch subagents, do not stall: a code-only corpus has no semantic work, so write the empty semantic file (Part B "Fast path") and continue to Part C; for a corpus with docs/papers/images, either set a MiniMax key or extract those inline yourself, but in no case prompt for `ANTHROPIC_API_KEY` — that prompt is a misread of this skill.
...
Before dispatching subagents, convert PDF files to markdown so inline `[IMAGE]` blocks carry vision-LLM descriptions (subagent reads as text, no vision needed). Each PDF may take up to 30 minutes depending on image count and vision API latency. To avoid bash-tool timeouts (OpenCode default 120s), run the conversion as a **detached background process** - the bash block returns in <1 second and the conversion continues until all PDFs are done. The script writes `_uncached.txt` after every PDF and the converter resumes from `.part.md` if interrupted, so re-running continues where it left off.
...
```bash
# Start-Process via PowerShell: spawn detached python and return immediately
...
if [ -f convert_all_pdfs.py ]; then
  powershell -NoProfile -Command "Start-Process python -ArgumentList @('-u','convert_all_pdfs.py','raw') -WorkingDirectory '$PWD' -RedirectStandardOutput '$LOG' -RedirectStandardError '$ERR' -WindowStyle Hidden -PassThru | Select-Object -ExpandProperty Id | Set-Content '$PID'"
  echo "B1.5 started in background (PID $(cat $PID 2>/dev/null)). Log: $LOG"
else
  echo "convert_all_pdfs.py not found - using inline loop (will hit 120s bash timeout on long PDFs)"
  $(cat graphify-out/.graphify_python) -u -c "
import sys
from pathlib import Path
...
if [ -n "$PID" ]; then
  # Use PowerShell to check if PID is still alive on Windows
  if powershell -NoProfile -Command "Get-Process -Id $PID -ErrorAction SilentlyContinue" 2>/dev/null | grep -q "Get-Process"; then
  ...
  echo ""
  echo "To stop: kill $PID   (or kill the python process in Task Manager)"
else
  echo "No background conversion running (.convert.pid not found)"
fi
```
